...
public interface SearchLogRepository extends JpaRepository<SearchLog, UUID> {
    @Query("SELECT s.queryText, COUNT(s) FROM SearchLog s WHERE s.tenantId = :tid AND s.zeroResult = true AND s.searchedAt >= :since GROUP BY s.queryText ORDER BY COUNT(s) DESC LIMIT 20")
